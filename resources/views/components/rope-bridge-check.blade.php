...
<div class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" wire:click="skipRopeBridge">
    <div class="bg-mountain-800 rounded-2xl border-2 border-trust-500 p-6 max-w-md w-full shadow-2xl animate-slide-up" wire:click.stop>
        <div class="text-center mb-4">
            ...
            <p class="text-mountain-300 text-sm mt-1">Mencoba naik ke level berikutnya</p>
        </div>
        ...
        @if(config("summit.thresholds.$thresholdKey"))
            ...
            <div class="grid {{ ($threshold['tt_required'] ?? true) ? 'grid-cols-3' : 'grid-cols-2' }} gap-3 mb-6">
                ...
                @if($threshold['tt_required'] ?? true)
                    <div class="text-center p-3 rounded-lg bg-mountain-900/50 border border-mountain-700">
                        <div class="text-xs text-mountain-400 mb-1">TT</div>
                        <div class="text-lg font-bold font-mono {{ $player->tt >= $threshold['tt'] ? 'text-trust-300' : 'text-crisis-400' }}">
