...
@foreach($levels as $idx=>$lvl)<div class="flex items-center"><div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold {{ $idx<$ci?'bg-camp-500 text-white':($idx===$ci?'bg-trust-500 text-mountain-950 ring-2 ring-trust-300':'bg-mountain-700 text-mountain-400') }}">{{ $idx===0?'B':($idx===1?'C':'S') }}</div>@if($idx<2)<div class="w-6 h-0.5 {{ $idx<$ci?'bg-camp-400':'bg-mountain-700' }}"></div>@endif</div>@endforeach
</div>
<div class="mb-2"><div class="flex justify-between text-xs mb-0.5"><span class="text-mountain-300">MP</span><span class="text-mountain-200 font-mono">{{ $mp }}</span></div><div class="w-full h-2 bg-mountain-800 rounded-full overflow-hidden"><div class="h-full bg-trust-400 rounded-full transition-all duration-500" style="width:{{ $mpPct }}%"></div></div></div>
<div class="mb-2"><div class="flex justify-between text-xs mb-0.5"><span class="text-mountain-300">SP</span><span class="text-mountain-200 font-mono">{{ $sp }}</span></div><div class="w-full h-2 bg-mountain-800 rounded-full overflow-hidden"><div class="h-full bg-trust-400 rounded-full transition-all duration-500" style="width:{{ $spPct }}%"></div></div></div>
<x-rope-meter :tt="$tt" :compact="$compact" />
@if($reputation != 0)
<div class="mt-1"><span class="text-xs {{ $reputation >= 0 ? 'text-camp-300' : 'text-crisis-400' }}">Reputasi: {{ $reputation >= 0 ? '+' : '' }}{{ $reputation }}</span></div>
@endif
