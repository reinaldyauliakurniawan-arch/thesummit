...
class TurnNotification extends Notification{use Queueable;
public function __construct(public GameRoom $room,public GamePlayer $player){}
public function via($n):array{return ['database'];}
public function toDatabase($n):array{return ['room_id'=>$this->room->id,'room_code'=>$this->room->code,'message'=>"Giliranmu di ruang {$this->room->code}!",'url'=>route('game.board',$this->room)];}
}
