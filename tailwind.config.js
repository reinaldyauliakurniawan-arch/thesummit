...
module.exports = {
  ...
  theme: {
    extend: {
      ...
      fontFamily:{expedition:["'Fjalla One'",'Georgia','Cambria','serif']},
      animation:{'card-flip':'cardFlip .6s ease-in-out','fade-in':'fadeIn .3s ease-in','slide-up':'slideUp .4s ease-out','pulse-gold':'pulseGold 2s infinite','badge-glow':'badgeGlow .8s ease-out'},
      keyframes:{
        cardFlip:{'0%':{transform:'rotateY(0deg)'},'50%':{transform:'rotateY(90deg)'},'100%':{transform:'rotateY(0deg)'}},
        fadeIn:{'0%':{opacity:'0'},'100%':{opacity:'1'}},
        slideUp:{'0%':{opacity:'0',transform:'translateY(20px)'},'100%':{opacity:'1',transform:'translateY(0)'}},
        pulseGold:{'0%,100%':{boxShadow:'0 0 0 0 rgba(245,158,11,0.4)',transform:'scale(1)'},'50%':{boxShadow:'0 0 0 8px rgba(245,158,11,0)',transform:'scale(1.02)'}},
        badgeGlow:{'0%':{transform:'scale(0.6)',opacity:'0',filter:'drop-shadow(0 0 0px rgba(245,158,11,0))'},'60%':{transform:'scale(1.1)',opacity:'1',filter:'drop-shadow(0 0 20px rgba(245,158,11,0.6))'},'100%':{transform:'scale(1)',opacity:'1',filter:'drop-shadow(0 0 8px rgba(245,158,11,0.3))'}},
      },
    },
  },
